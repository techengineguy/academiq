...
<div class="space-y-6 py-4">
    <div class="flex items-start justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('Communicate with teachers and school staff.') }}</p>
        </div>
        <flux:button class="button" href="{{ route('parent.messages.create') }}" wire:navigate icon="plus">
            {{ __('Compose') }}
        </flux:button>
    </div>
        ...
        @if($this->messages->count())
            <div class="space-y-2">
                @foreach($this->messages as $message)
                    <a href="{{ route('parent.messages.show', $message->id) }}" wire:navigate
